-- Parallel variant: Compile and run with:
-- stack ghc -- -threaded -o ../aoc -O2 aoc13parallel && time ../aoc +RTS -N70 -RTS aoc13.dat

import Control.Concurrent (forkIO, killThread, newEmptyMVar, readMVar, tryPutMVar)
...
main = do
  ...
  withFile filename ReadMode $ \file -> do
    ...
    print (busKs', busJs')
    let (busKs, temp) = splitAt 2 busKs'
    let busJs = temp ++ busJs'
    print (busKs, busJs)
    ...
    let parFind :: Int -> [(Int, Int)] -> IO Int
        parFind n busJs = do
          resultV <- newEmptyMVar
          threads <- forM [1..n] $ \i -> forkIO $ do
            void $ tryPutMVar resultV $! head (filter (condAll busJs) [i*pBusKs, (n+i)*pBusKs..]) - bus0
          result <- readMVar resultV
          forM_ threads killThread
          return result
          
    t <- parFind GHC.Conc.numCapabilities busJs
    print t
